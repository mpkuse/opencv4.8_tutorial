...
namespace helpers
{
  ...
  void MiscUtils::point2f_2_eigen(const std::vector<cv::Point2f> &kp, MatrixXd &uv, bool make_homogeneous)
  {
    assert(kp.size() > 0 && "MiscUtils::keypoint_2_eigen empty kp.");
    uv = MatrixXd::Constant((make_homogeneous ? 3 : 2), kp.size(), 1.0);
    for (int i = 0; i < kp.size(); i++)
  ...
  void MiscUtils::keypoint_2_point2f(const std::vector<cv::KeyPoint> &kp, std::vector<cv::Point2f> &kp_2f)
  {
    kp_2f.clear();
    for (const auto &keypoint : kp)
    {
      kp_2f.push_back(keypoint.pt);
    }
  }

  void MiscUtils::eigen_2_point2f(const MatrixXd &uv, std::vector<cv::Point2f> &kp_2f)
  {
    assert(uv.rows() == 2 || uv.rows() == 3);
    kp_2f.clear();
    for (uint32_t i = 0; i < uv.cols(); i++)
    {
      auto pt = cv::Point2f(uv(0, i), uv(1, i));
      kp_2f.push_back(pt);
    }
  }

  uint32_t MiscUtils::count_inliers(const std::vector<uchar> &inliers)
  {
    uint32_t n_inliers = 0u;
    for (const auto &u : inliers)
    {
      if (u > 0u)
        n_inliers++;
    }
  ...
  void MiscUtils::retain_inliers(const std::vector<cv::Point2f> &kp, const std::vector<uchar> &inliers, std::vector<cv::Point2f> &kp_retained)
  {
    kp_retained.clear();
    assert(kp.size() == inliers.size() && "Expecting number of keypoints to be equal to inliers mask");
  ...
  void MiscUtils::retain_non_inliers(const std::vector<cv::Point2f> &kp, const std::vector<uchar> &inliers, std::vector<cv::Point2f> &kp_retained)
  {
    kp_retained.clear();
    assert(kp.size() == inliers.size() && "Expecting number of keypoints to be equal to inliers mask");
    for (auto i = 0u; i < kp.size(); i++)
  ...
  void MiscUtils::plot_point_sets(const cv::Mat &im, const cv::Mat &pts_set, cv::Mat &dst, const cv::Scalar &color, bool enable_keypoint_annotation, const string &msg)
  {
    ...
    if (im.data == dst.data)
    { // this is pointer comparison to know if this operation is inplace
                                  ...
                                  vector<cv::Scalar> &color_annotations, float alpha, const string &msg)
  {
    ...
    for (int i = 0; i < pts_set.cols(); i++)
    {
      if (pts_set(0, i) < 0 || pts_set(0, i) > im.cols || pts_set(1, i) < 0 || pts_set(1, i) > im.rows)
      { // avoid points which are outside
                                  ...
                                  const string &msg)
  {
    ...
    if (msg.length() > 0)
    { // ':' separated. Each will go in new line
                                  ...
                                  const string &msg)
  {
    ...
    for (int kl = 0; kl < ptsA.cols(); kl++)
    {
      ...
      if (color_map_direction == 2)
        coloridx = (int)((ptsA(0, kl) + ptsA(1, kl)) / (imA.rows + imA.cols) * 256.); // manhattan-gradiant
      if (color_map_direction == 3)
        coloridx = (int)(abs(ptsA(0, kl) - imA.rows / 2. + ptsA(1, kl) - imA.cols / 2.) / (imA.rows / 2. + imA.cols / 2.) * 256.); // image centered manhattan-gradiant
